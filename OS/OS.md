...
| **면접 포인트** | “멀티프로세스 vs 멀티스레드” | 트레이드오프 언급 — 안정성(프로세스) vs 효율(스레드) |
...
| **면접 포인트** | “실시간 서비스에서 어떤 스케줄링?” | Round Robin + Priority 조합 언급 가능 |
...
| **면접 포인트** | “서버 메모리 부족 시 어떤 현상?” | → 스와핑(swap) 발생, CPU I/O 대기 ↑ |
- 페이징 - 고정 - 내부 단편화
...
면접 포인트: “`sleep()` 호출 시 어떤 상태?” → 대기 상태(Waiting).
...
| **면접 포인트** | “사용자 모드와 커널 모드 전환 비용” | “컨텍스트 스위칭 발생, 최소화 중요” |

## 질문

---

실제 리눅스, 윈도우에 대한 스케줄링 알고리즘은??

- 윈도우즈에서는 스레드 단위로 스케줄링이 이루어진다.
    - 우선순위 기반 + 라운드로빈 시간할당 + 다단계 큐(멀티레벨) 개념이 섞여있는 알고리즘을 사용함
    - 마이크로소프트 공식 스케줄링 관련 docs
        - [https://learn.microsoft.com/en-us/windows/win32/procthread/scheduling?utm_source=chatgpt.com](https://learn.microsoft.com/en-us/windows/win32/procthread/scheduling?utm_source=chatgpt.com)
- 리눅스에서는 기본 스케줄러가 CFS (Completely Fair Scheduler)

컨텍스트 스위칭이 CPU 스케줄링 알고리즘에서 발생??

- 발생함
    - CPU 스케줄러가 “지금 실행 중인 스레드/프로세스를 멈추고(또는 대기 상태로 바꾸고) 다른 스레드를 실행해야겠다”고 결정할 때, 실행 중이던 스레드의 상태(context: 레지스터 값, 스택 포인터, 프로세스/스레드 상태 등)를 저장(saving)하고, 새로 실행될 스레드의 상태를 복원(restoring)하는 작업이 필요하다. 이 작업이 **컨텍스트 스위칭**이다.